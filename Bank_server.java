...
class BankServer extends Thread {
	...
	private MimeMessage draftEmail(int accno,int amt,String mode,String emailid)  {
		...
		String emailSubject = "ABC Bank \n\nAmount Transaction";
		String emailBody="";
		String emailBody1,emailBody2;
		
		if(mode.equals("w"))
		{
			emailBody1 = "Your A/C no."+accno+" is withdrawed for Rs."+amt+" towards Medical collage branch on "+day.format(date)+" at "+time.format(date);
			emailBody2="\n\n\n\n\nThis is a system generated mail,do not reply to this mail.";
			emailBody=emailBody1+emailBody2;
		}
		else
		{
			emailBody1= "Your A/C no."+accno+" is deposited for Rs."+amt+" towards Medical collage branch on "+day.format(date)+" at "+time.format(date);	
			emailBody2="\n\n\n\n\nThis is a system generated mail,do not reply to this mail.";
			emailBody=emailBody1+emailBody2;
		}
	...
	private void setupServerProperties() {
	 ...
	 public void managedb() 
	    {
	    	String jdbcurl="jdbc:sqlite:/D:\\SqLite\\sqlite-tools-win32-x86-3360000\\AccountDetails.db";
	    	try {
	 ...
	 private String decrypt(String pt,Key key) throws Exception 
		{
		Cipher c=Cipher.getInstance("AES");
